...
fun SPHome(navController: NavController, viewModel: SPHomeViewModel ) {
    ...
    // More reliable way to handle navigation results
    val currentBackStackEntry by navController.currentBackStackEntryAsState()
    
    LaunchedEffect(currentBackStackEntry) {
        currentBackStackEntry?.savedStateHandle?.let { savedStateHandle ->
            val result = savedStateHandle.get<Map<String, Any>>("service_submission_result")
            result?.let {
                ...
                // Clear the result immediately after processing
                savedStateHandle.remove<Map<String, Any>>("service_submission_result")
            }
        }
    }
